...
int main(int argc, char *argv[])
{
	...
	cLog << "M100 config is valid and program run successfully." << endl;

	if (!Startup()) // Extend function
	{
		cLog << "Program exited. Startup function return error." << endl;
		return ERR_STARTUP;
	}
	...
	else // Load SS successfully
	{
		...
		if (!Active(sdkScript))
		{
			cLog << "Could not Active. Program exited." << endl;
			return ERR_ACTIVE;
		}
	}

	cLog << "Load key and active successfully." << endl;
	...
	{
		cLog << "Program exited. Cleanup function return error." << endl;
		return ERR_CLEANUP;
	}
