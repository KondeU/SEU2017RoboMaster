...
void CArmor::FindArmorBase(Mat mImg, Point & tArmor)
{
	...
	for (size_t ui1 = 0; ui1 < vContours.size(); ui1++)
	{
		for (size_t ui2 = ui1 + 1; ui2 < vContours.size(); ui2++)
		{
			...
			double dMaxRect1 = max(vRectContours[ui1].size.width, vRectContours[ui1].size.height);
			double dMinRect1 = min(vRectContours[ui1].size.width, vRectContours[ui1].size.height);
			
			double dMaxRect2 = max(vRectContours[ui2].size.width, vRectContours[ui2].size.height);
			double dMinRect2 = min(vRectContours[ui2].size.width, vRectContours[ui2].size.height);
			
			if ((fabs(dMaxRect1 - dMaxRect2)) > iContoursRectMaxSideDiffLimit)
				continue;

			if ((fabs(dMinRect1 - dMinRect2)) > iContoursRectMinSideDiffLimit)
				continue;
			
			if ((dMaxRect1 / dMinRect1) < dContoursRectAspectRatioLimitSmall ||
				(dMaxRect1 / dMinRect1) > dContoursRectAspectRatioLimitLarge ||
				(dMaxRect2 / dMinRect2) < dContoursRectAspectRatioLimitSmall ||
				(dMaxRect2 / dMinRect2) > dContoursRectAspectRatioLimitLarge)
				continue;
			...
			for (size_t ui = 0; ui < vLight.size(); ui++)
			{
				...
				if (vLight[ui] == vContoursCentroid[ui2])
				{
					bHaveAdd2 = true;
				}
			...
			//cout << "DEBUG(1):vLight.size()=" << vLight.size() << endl; // Armor debug
		}
	}
	
	//cout << "DEBUG5" << endl; // Armor debug
