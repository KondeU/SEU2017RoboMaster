...
CSerial::CSerial() : cLog("Serial")
{
	...
	cLog << prefix << "Serial(UART3) to STM32: " << strerror(errno) << endl;
...
void CSerial::SetLight(byte byLight, int iColor)
{
	switch (iColor)
	{
	case LGCLR_NONE:
		SetLight(byLight, 0, 0, 0);
		break;
	
	case LGCLR_RED:
		SetLight(byLight, 255, 0, 0);
		break;
		
	case LGCLR_GREEN:
		SetLight(byLight, 0, 255, 0);
		break;
		
	case LGCLR_BLUE:
		SetLight(byLight, 0, 0, 255);
		break;
		
	case LGCLR_YELLOW:
		SetLight(byLight, 255, 255, 0);
		break;
		
	case LGCLR_WHITE:
		SetLight(byLight, 255, 255, 255);
		break;
		
	case LGCLR_CYAN:
		SetLight(byLight, 0, 255, 255);
		break;
	}
}
...
bool CSerial::Send()
{
	...
	if (iWrited == -1)
	{
		cLog << prefix << "Write data to serial failed." << endl;
		return false;
	}
...
bool CSerial::Recv()
{
	...
	while (iReaded < int(sizeof(tFrame)))
	{
		...
		#if DEBUG_SERIAL
		cLog << prefix << "Serial recv used time:" << dUsedTime << endl;
		#endif
	...
	#if DEBUG_SERIAL
	cLog << prefix << "Read data from serial successfully." << endl;
	#endif
