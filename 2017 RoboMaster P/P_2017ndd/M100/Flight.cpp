...
CFlight::CFlight(ConboardSDKScript * pst) : pScript(pst)
{
	#ifdef MODE_AUTOSTART
	fout.open("./autorun/2017RobomasterTask/Runlog/Flight.log");
	#else
	fout.open("./Runlog/Flight.log");
	#endif
}
...
float CFlight::CalcYaw(const double & x, const double & y)
{
	...
	if (0 == y)
	{
		...
		{
			cerr << "$ Error : In function CalcYaw, class CFlight : Both x and y value is zero, now set yaw with 0." << endl;
			fout << "Error : In function CalcYaw, class CFlight : Both x and y value is zero, now set yaw with 0.\n" << endl;
			fYaw = 0;
		}
	}
	else
...
void CFlight::CtrlObtain()
{
	...
	cout << "$ Obtain aircraft control." << endl;
	fout << "Obtain aircraft control.\n" << endl;
	sleep(1);
}
...
void CFlight::CtrlRelease()
{
	...
	cout << "$ Release aircraft control." << endl;
	fout << "Release aircraft control.\n" << endl;
	sleep(1);
}
...
void CFlight::Takeoff()
{
	...
	sleep(3);
	#else
	cout << "$ Virtual take off!" << endl;
	fout << "Virtual take off!\n" << endl;
	#endif
}
